...
# 另一个进程（test/run_target_waypoints.sh）经由 UDP 127.0.0.1:14600 发布。
# --duration 0 表示一直跟踪到 Ctrl-C 或目标状态超时。
#
# 常用示例：
#   ./test/run_tracker.sh --execute                     # 锁存当前相对位置持续跟踪
#   ./test/run_tracker.sh --offset-east -3 --execute    # 指定站位：目标东側 3 m
