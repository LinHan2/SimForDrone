...
等待两套 PX4 都打印 `Ready for takeoff!`。该终端必须持续运行；停止场景时在此终端按
`Ctrl-C`。
...
先在终端 2 启动 target dry-run，它会连续发布 15 秒共享状态：
...
两个命令均不带 `--execute`，不会解锁。成功标志是 tracker 输出
`DRY RUN PASS: tracker 与 target 状态流已就绪`。

## 3. 单机 target 航点

在通过 dry-run 后，先只飞 target，确认本场景的控制环仍正常：
...
只有步骤 3 正常完成后才启动。终端 2 先启动交互 target：
